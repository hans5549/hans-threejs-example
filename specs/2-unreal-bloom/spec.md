...
- 當 3D 模型載入失敗時，顯示友善的錯誤訊息並提供「重新載入」按鈕
- 當瀏覽器不支援 WebGL 時，提供 2D 降級版本的簡化展示（無 3D 效果）
...
- **FR-005**: 系統 MUST 提供 GUI 控制面板來調整 Bloom 參數，預設展開顯示所有控制項
...
- Q: 當 3D 模型載入失敗時，系統應如何處理？ → A: 顯示友善的錯誤訊息，並提供「重新載入」按鈕讓使用者重試
- Q: 當使用者的瀏覽器不支援 WebGL 時，系統應顯示什麼內容？ → A: 提供 2D 降級版本的簡化展示（無 3D 效果）
- Q: 本功能需要支援哪些瀏覽器？ → A: 僅現代瀏覽器（Chrome、Firefox、Safari、Edge 最新兩個主要版本）
- Q: 3D 模型應該從哪裡載入？ → A: 從 Three.js 官方範例 CDN 載入（與官方範例相同來源）
- Q: GUI 控制面板在頁面載入時應該預設展開還是收合？ → A: 預設展開，所有控制項目立即可見
...
- 3D 模型來源：從 Three.js 官方範例 CDN 載入 PrimaryIonDrive.glb
- 目標瀏覽器：僅現代瀏覽器（Chrome、Firefox、Safari、Edge 最新兩個主要版本）
