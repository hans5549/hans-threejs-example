...
- 當瀏覽器不支援 WebGL 時，顯示靜態地球圖片作為降級方案
- 當視窗大小改變時，畫面應該如何調整？應自動調整渲染尺寸以適應新視窗大小
- 當滑鼠移出視窗時，攝影機會緩慢平滑地回到預設的正面中央視角
...
- **Earth Texture（地球紋理）**: NASA Blue Marble 地球表面衛星影像（公共領域），用於決定每個粒子的顏色
...
- **SC-002**: 粒子數量為 50,000 個，使用斐波那契球面分布以清晰呈現地球輪廓
...
- Q: 粒子應如何分布在球面上？ → A: 使用斐波那契球面分布（黃金角演算法），確保視覺上均勻分布
- Q: 滑鼠移出視窗時攝影機如何行為？ → A: 緩慢回到預設位置（正面中央視角）
- Q: 地球紋理圖的來源為何？ → A: 使用 NASA Blue Marble 高解析度地球衛星影像（公共領域）
- Q: WebGL 不支援時如何處理？ → A: 顯示靜態圖片作為降級方案
- Q: 粒子數量的精確目標為何？ → A: 50,000 個粒子（平衡視覺效果與效能）
