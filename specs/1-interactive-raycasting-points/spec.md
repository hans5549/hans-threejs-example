...
- 當瀏覽器不支援 WebGL 時，應如何處理？（假設：顯示錯誤訊息或降級處理）
- 當滑鼠快速移動時，偵測的點是否會閃爍或跳動？（假設：使用合理的更新頻率避免過度敏感）
- 當視窗非常小時，點雲是否仍然可見且可互動？（假設：維持最小可用尺寸）
...
- 使用者的瀏覽器支援 WebGL 2.0 或更高版本
- 使用者的設備具備基本的 3D 渲染能力
- 網路連線正常，可以載入 Three.js 函式庫（如使用 CDN）
