...
- Q: 3D 模型檔案的具體來源策略？ → A: 使用 Three.js 官方範例的模型路徑（如 `models/fbx/stanford-bunny.fbx`）
- Q: 模型載入完成前使用者看到的初始畫面內容？ → A: 顯示空場景（背景、光照）加載入進度指示器（百分比或動畫）
- Q: 光線數量「過高」和「合理範圍」的具體數值定義？ → A: 無硬上限，僅在 FPS 低於 30 時顯示警告（動態調整策略）
- Q: 載入逾時的具體時間和失敗後的恢復策略？ → A: 60 秒逾時，顯示錯誤訊息但不提供重試（需重新整理頁面）
- Q: 光線的空間分布策略？ → A: 在模型周圍球體表面隨機取樣，光線指向模型中心區域
...
- 當模型尚未載入完成時，如何處理使用者的互動操作？（系統應顯示空場景加載入進度指示器，並禁用互動控制）
- 當光線數量設定過高導致效能嚴重下降時如何提示使用者？（在 FPS 低於 30 時顯示效能警告訊息，建議減少光線數量以維持流暢體驗）
- 當瀏覽器不支援 WebGL 時如何優雅降級？（顯示友善的錯誤訊息和系統需求）
- 當視窗大小改變時如何調整渲染器和相機？（自動調整比例和重新渲染）
- 當模型載入超過 60 秒或失敗時如何處理？（顯示錯誤訊息，使用者需重新整理頁面重試）

## 需求 *(必填)*

### 功能需求

- **FR-001**: 系統必須能夠從 Three.js 官方範例路徑（`models/fbx/stanford-bunny.fbx`）載入 FBX 格式的 3D 模型並在場景中正確顯示
- **FR-002**: 系統必須為載入的模型幾何體建立 BVH 加速結構
- **FR-003**: 系統必須能夠從模型周圍球體表面的隨機位置投射光線，光線方向指向模型中心區域，投射數量可調整（預設 100 條）
...
- **FR-011**: 系統必須提供控制介面讓使用者調整光線數量（無硬上限）
...
- **FR-016**: 系統必須在模型載入前顯示空場景（背景、光照）和載入進度指示器
- **FR-017**: 系統必須在 FPS 低於 30 時顯示效能警告訊息
- **FR-018**: 系統必須在模型載入超過 60 秒時顯示逾時錯誤訊息

### 關鍵實體 *(當功能涉及資料時包含)*

- **3D 模型**: 用於光線投射測試的幾何體（Stanford Bunny FBX 模型），包含頂點位置、法線等資料
- **BVH 樹**: 分層包圍盒結構，用於加速空間查詢，包含節點、包圍盒資訊
- **光線**: 從模型周圍球體表面隨機位置發射的射線，方向指向模型中心區域，包含原點、方向、最大距離
- **相交結果**: 光線與模型相交的資訊，包含相交點座標、距離、面索引
- **效能指標**: 即時統計資料，包含 FPS、幀時間、記憶體使用量，當 FPS 低於 30 時觸發警告
- **載入狀態**: 模型載入進度資訊，包含百分比或動畫指示器，逾時設定為 60 秒
...
1. **效能風險**: 在低階裝置上，即使使用 BVH，過多光線仍可能導致效能問題
   - 緩解措施：在 FPS 低於 30 時自動顯示效能警告，建議減少光線數量

2. **模型載入風險**: 大型 FBX 檔案可能載入緩慢或失敗
   - 緩解措施：設定 60 秒載入逾時，顯示載入進度指示器，逾時後顯示錯誤訊息
...
5. **視覺雜訊**: 過多光線和相交點可能導致畫面過於複雜難以理解
   - 緩解措施：使用半透明材質和合理的預設光線數量（100 條）

6. **網路中斷**: 模型載入過程中網路中斷可能導致載入失敗
   - 緩解措施：在逾時後顯示明確錯誤訊息，引導使用者重新整理頁面
