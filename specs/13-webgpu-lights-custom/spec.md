...
- 當瀏覽器不支援 WebGPU 時，顯示靜態圖片代替互動場景，並附帶說明文字
...
- Q: 當瀏覽器不支援 WebGPU 時的處理方式？ → A: 使用靜態圖片代替互動場景
- Q: 點雲粒子的視覺呈現？ → A: 預設白色粒子，使用 PointsNodeMaterial 預設點大小
- Q: 點雲的空間分佈範圍？ → A: 3x3x3 立方體（中心在原點，邊長 3 單位）
- Q: 光源的有效照射範圍？ → A: 距離 1 單位，光源影響範圍約佔點雲的 1/3
- Q: 場景背景顏色？ → A: 純黑色背景（0x000000）
